...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        // Llamar a la función para obtener los nombres de las clínicas
        $query = "SELECT * FROM obtener_nombres_clinicas();";
        ...
        $clinicasConId = array_map(function($clinica, $index) {
            return [
                'id' => $index + 1, // Enumeración comenzando desde 1
                'nombre' => $clinica['nombre']
            ];
        }, $clinicas, array_keys($clinicas));

        // Retornar los resultados en formato JSON
        echo json_encode($clinicasConId);
    } else {
